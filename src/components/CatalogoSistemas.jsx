const CatalogoSistemas = () => {
  ...
  return (
    <div className="h-full py-12 flex flex-col mb-12 items-center bg-slate-50 rounded-sm  w-full   ">
      ...
      <div className="flex flex-cols-2 md:flex-cols-3 gap-4 sm:gap-14  stify-evenly   min-w-80  h-full">
        {items.map((item, index) => (
             <div
             key={index}
             className=" md:shadow-md  rounded-md flex flex-col h-[10rem] w-[10rem] md:w-[18rem] md:h-[20rem] sm:h-[10rem] sm:w-[10rem]  items-center justify-center"
           >
